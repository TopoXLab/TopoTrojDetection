...
class my_Patch_PairDataset(data.Dataset):
    def __init__(self,
                 data_dir,
                 num_example=10):
        ...
        self.orig_labels = []
        self.target_labels = []
        r = re.compile(".*_([0-9]*).png")

        train_file_path = os.path.join(data_dir, "mnist_alphatrigger_0.2_experiment_train.csv")
        if not os.path.exists(train_file_path):
            train_file_path = os.path.join(data_dir, "data/mnist_alphatrigger_0.2_experiment_train.csv")
        if not os.path.exists(train_file_path):
            train_file_path = os.path.join(data_dir, "mnist_alphatrigger_0.0_experiment_train.csv")
        if not os.path.exists(train_file_path):
            train_file_path = os.path.join(data_dir, "data/mnist_alphatrigger_0.0_experiment_train.csv")
        train_file = pd.read_csv(train_file_path, sep=",")

        clean_files = train_file['file'][train_file['triggered'] == False].tolist()
        triggered_files = train_file['file'][train_file['triggered'] == True].tolist()
        if not len(triggered_files):
            triggered_files = clean_files

        orig_files = train_file['file'].tolist()
        orig_labels_list = train_file['true_label'].tolist()
        train_labels_list = train_file['train_label'].tolist()

        ind_list = torch.randperm(len(orig_files))
        count = 0
        self.triggered_count = 0
        #         selected_classes=train_file['true_label'][train_file['triggered']==True].unique().tolist()
        #         if not len(selected_classes):
        #             selected_classes=[x for x in range(10)]
        #         else:
        #             selected_classes=np.random.choice(range(10), 1).tolist()

        for i in ind_list:
            if count >= num_example:
                break
            image_class = r.findall(orig_files[i].split("/")[-1])
            #             if not (int(image_class[0]) in selected_classes):
            #                 continue

            self.orig_labels.append(orig_labels_list[i])
            self.target_labels.append(train_labels_list[i])

            if train_file['triggered'].iloc[int(i)] == True:
                img_c_file = os.path.join(data_dir, 'data',
                                          os.path.join(*clean_files[0].split("/")[:-1]),
                                          orig_files[i].split("/")[-1])
                if not os.path.exists(img_c_file):
                    img_c_file = os.path.join(data_dir,
                                              os.path.join(*clean_files[0].split("/")[:-1]),
                                              orig_files[i].split("/")[-1])
                img_c = skimage.io.imread(img_c_file)

                img_t_file = os.path.join(data_dir, 'data', orig_files[i])
                if not os.path.exists(img_t_file):
                    img_t_file = os.path.join(data_dir, orig_files[i])
                img_t = skimage.io.imread(img_t_file)
            else:
                img_c_file = os.path.join(data_dir, 'data', orig_files[i])
                if not os.path.exists(img_c_file):
                    img_c_file = os.path.join(data_dir, orig_files[i])
                img_c = skimage.io.imread(img_c_file)

                img_t_file = os.path.join(data_dir, 'data',
                                          os.path.join(*triggered_files[0].split("/")[:-1]),
                                          orig_files[i].split("/")[-1])
                if not os.path.exists(img_t_file):
                    img_t_file = os.path.join(data_dir,
                                              os.path.join(*triggered_files[0].split("/")[:-1]),
                                              orig_files[i].split("/")[-1])
                if os.path.exists(img_t_file):
                    img_t = skimage.io.imread(img_t_file)
                else:
                    img_t = img_c.copy()

            img_t = torch.tensor(img_t).unsqueeze(0).float()
            img_c = torch.tensor(img_c).unsqueeze(0).float()
            self.image_troj.append(img_t)
            self.image_clean.append(img_c)
            count += 1

    def __len__(self):
        return len(self.image_clean)

    def __getitem__(self, index):
        return index, self.image_clean[index], self.image_troj[index], self.orig_labels[index], self.target_labels[
            index]

    def select_data(self, select_ind):
        self.image_clean = [self.image_clean[i] for i in select_ind]
        self.image_troj = [self.image_troj[i] for i in select_ind]
        self.orig_labels = [self.orig_labels[i] for i in select_ind]
        self.target_labels = [self.target_labels[i] for i in select_ind]

    def update_data(self, select_ind, new_data):
        for i in range(len(select_ind)):
            self.image_clean[select_ind[i]] = new_data[i]
        self.select_data(select_ind)
